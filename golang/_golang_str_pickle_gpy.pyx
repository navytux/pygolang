...
"""_golang_str_pickle_gpy.pyx complements _golang_str_pickle.pyx and keeps
everything related to patching python runtime to load pickle *STRING opcodes as
bstr, and to save back bstr via *STRING opcodes as well.

It is included from _golang_str_pickle.pyx .

The main entry-points are _patch_str_pickle and _patch_capi_unicode_decode_as_bstr.

See also _golang_str_gpy.pyx and _golang_str_pickle_gpy.S .
"""
...
cdef extern from * nogil:
    ...
    // FOR_EACH_CALLCONV(X) invokes macro X(ccname, callconv, cckind) for every supported calling convention.
    // cckind is one of the following:
    //
    // - `builtin` represents native calling conventions of the compiler
    //    available to the programmer via function attributes.
    // - `builtin_mod` is like builtin, but indicates that save has
    //    additional PickleState* argument.
    ...
    // - `builtin_psave0` is like builtin, but indicates
    //    that `pers_save` argument of `save` was const-propagated with `pers_save=0`.
    // - `builtin_mod_psave0` is builtin_mod + builtin_psave0.
    //
    ...
    #  define FOR_EACH_CALLCONV(X)  \
         ...
         X(default_mod,,                                builtin_mod)            \
         X(cdecl_mod,           CALLCONV(cdecl),        builtin_mod)            \
         X(stdcall_mod,         CALLCONV(stdcall),      builtin_mod)            \
         X(fastcall_mod,        CALLCONV(fastcall),     builtin_mod)            \
         X(thiscall_mod,        CALLCONV(thiscall),     builtin_mod)            \
         X(regparm1_mod,        CALLCONV(regparm(1)),   builtin_mod)            \
         X(regparm2_mod,        CALLCONV(regparm(2)),   builtin_mod)            \
         X(regparm3_mod,        CALLCONV(regparm(3)),   builtin_mod)            \
         X(fastcall_nostkclean, na,                     custom )                \
         /* XXX fastcall_nostkclean_mod                                     */  \
         ...
         X(default_mod_psave0,,                         builtin_mod_psave0)     \
         X(cdecl_mod_psave0,    CALLCONV(cdecl),        builtin_mod_psave0)     \
         X(stdcall_mod_psave0,  CALLCONV(stdcall),      builtin_mod_psave0)     \
         X(fastcall_mod_psave0, CALLCONV(fastcall),     builtin_mod_psave0)     \
         X(thiscall_mod_psave0, CALLCONV(thiscall),     builtin_mod_psave0)     \
         X(regparm1_mod_psave0, CALLCONV(regparm(1)),   builtin_mod_psave0)     \
         X(regparm2_mod_psave0, CALLCONV(regparm(2)),   builtin_mod_psave0)     \
         X(regparm3_mod_psave0, CALLCONV(regparm(3)),   builtin_mod_psave0)
    # else // MSC
    #  define FOR_EACH_CALLCONV(X)  \
         X(default,,                                    builtin)                \
         X(cdecl,               CALLCONV(cdecl),        builtin)                \
         ...
         X(fastcall_mod,        CALLCONV(fastcall),     builtin_mod)            \
         /* X(thiscall_mod,     CALLCONV(thiscall),     builtin_mod)        */  \
         X(vectorcall_mod,      CALLCONV(vectorcall),   builtin_mod)            \
         X(fastcall_nostkclean, na,                     custom )                \
         /* XXX fastcall_nostkclean_mod                                     */  \
         X(default_psave0,,                             builtin_psave0)         \
         X(cdecl_psave0,        CALLCONV(cdecl),        builtin_psave0)         \
         ...
         X(default_psave0,,                             builtin_mod_psave0)     \
         X(cdecl_psave0,        CALLCONV(cdecl),        builtin_mod_psave0)     \
         X(stdcall_psave0,      CALLCONV(stdcall),      builtin_mod_psave0)     \
         X(fastcall_psave0,     CALLCONV(fastcall),     builtin_mod_psave0)     \
         /* X(thiscall_psave0,  CALLCONV(thiscall),     builtin_mod_psave0) */  \
         X(vectorcall_psave0,   CALLCONV(vectorcall),   builtin_mod_psave0)
    # endif
    ...
    # define FOR_EACH_CALLCONV(X)   \
        ...
        X(default_mod,,                                 builtin_mod)            \
        X(default_psave0,,                              builtin_psave0)         \
        X(default_mod_psave0,,                          builtin_psave0)
    #else
    ...
    // SaveFunc represents a save function - its address and calling convention.    XXX + argv_kind
    struct SaveFunc {
        void*     addr;
        Callconv  cconv;
    };
...
cdef _patch_str_pickle():
    ...
    # py3.11-: pickletools.dis raises UnicodeDecodeError on non-ascii STRING and treats *BINSTRING as latin1
    #          -> decode as UTF8b instead
    # py3.12+: pickletools.dis explicitly uses ascii(arg) for argument of *STRING
    #          -> injecting patched ascii to pickledis that still calls full repr on bstr
...
cdef _patch_cpickle(_pickle, _pickle_PatchCtx *pctx):
    ...
    pctx.Pickler_save_orig = save = _find_Pickler_save(_pickle, _pickle.Pickler)
...
cdef int _pickle_Pickler_xsave(PickleState* mod, PicklerObject* self, PyObject* obj, int pers_save) except -1:
    return __Pickler_xsave(&_pickle_patchctx, mod, self, obj, pers_save)

cdef int _zpickle_Pickler_xsave(PickleState* mod, PicklerObject* self, PyObject* obj, int pers_save) except -1:
    return __Pickler_xsave(&_zpickle_patchctx, mod, self, obj, pers_save)

cdef int _pickle_Pickler_xsave_psave0(PickleState* mod, PicklerObject* self, PyObject* obj) except -1:
    return __Pickler_xsave_psave0(&_pickle_patchctx, mod, self, obj)

cdef int _zpickle_Pickler_xsave_psave0(PickleState* mod, PicklerObject* self, PyObject* obj) except -1:
    return __Pickler_xsave_psave0(&_zpickle_patchctx, mod, self, obj)

# callconv wrappers XXX place
cdef extern from *:
    r"""
    static int __pyx_f_6golang_7_golang__pickle_Pickler_xsave(PickleState*, PicklerObject*, PyObject*, int);
    static int __pyx_f_6golang_7_golang__zpickle_Pickler_xsave(PickleState*, PicklerObject*, PyObject*, int);
    ...
    _pickle_Pickler_xsave_##ccname(PicklerObject* self, PyObject* obj, int pers_save) {     \
        return __pyx_f_6golang_7_golang__pickle_Pickler_xsave(NULL, self, obj, pers_save);  \
    }
    ...
    _zpickle_Pickler_xsave_##ccname(PicklerObject* self, PyObject* obj, int pers_save) {    \
        return __pyx_f_6golang_7_golang__zpickle_Pickler_xsave(NULL, self, obj, pers_save); \
    }

    #define DEF_PICKLE_XSAVE_builtin_mod(ccname, callconv)                                                  \
    static int callconv                                                                                     \
    _pickle_Pickler_xsave_##ccname(PickleState* mod, PicklerObject* self, PyObject* obj, int pers_save) {   \
        return __pyx_f_6golang_7_golang__pickle_Pickler_xsave(mod, self, obj, pers_save);                   \
    }
    #define DEF_ZPICKLE_XSAVE_builtin_mod(ccname, callconv)                                                 \
    static int callconv                                                                                     \
    _zpickle_Pickler_xsave_##ccname(PickleState* mod, PicklerObject* self, PyObject* obj, int pers_save) {  \
        return __pyx_f_6golang_7_golang__zpickle_Pickler_xsave(mod, self, obj, pers_save);                  \
    }


    static int __pyx_f_6golang_7_golang__pickle_Pickler_xsave_psave0(PickleState*, PicklerObject*, PyObject*);
    static int __pyx_f_6golang_7_golang__zpickle_Pickler_xsave_psave0(PickleState*, PicklerObject*, PyObject*);
    #define DEF_PICKLE_XSAVE_builtin_psave0(ccname, callconv)                               \
    static int callconv                                                                     \
    _pickle_Pickler_xsave_##ccname(PicklerObject* self, PyObject* obj) {                    \
        return __pyx_f_6golang_7_golang__pickle_Pickler_xsave_psave0(NULL, self, obj);      \
    }
    #define DEF_ZPICKLE_XSAVE_builtin_psave0(ccname, callconv)                              \
    static int callconv                                                                     \
    _zpickle_Pickler_xsave_##ccname(PicklerObject* self, PyObject* obj) {                   \
        return __pyx_f_6golang_7_golang__zpickle_Pickler_xsave_psave0(NULL, self, obj);     \
    }

    #define DEF_PICKLE_XSAVE_builtin_mod_psave0(ccname, callconv)                           \
    static int callconv                                                                     \
    _pickle_Pickler_xsave_##ccname(PickleState* mod, PicklerObject* self, PyObject* obj) {  \
        return __pyx_f_6golang_7_golang__pickle_Pickler_xsave_psave0(mod, self, obj);       \
    }
    #define DEF_ZPICKLE_XSAVE_builtin_mod_psave0(ccname, callconv)                          \
    static int callconv                                                                     \
    _zpickle_Pickler_xsave_##ccname(PickleState* mod, PicklerObject* self, PyObject* obj) { \
        return __pyx_f_6golang_7_golang__zpickle_Pickler_xsave_psave0(mod, self, obj);      \
    }
...
cdef int __Pickler_xsave(_pickle_PatchCtx* pctx, PickleState* mod, PicklerObject* self, PyObject* obj, int pers_save) except -1:
    ...
    #
    # we are ok to adjust .pers_save because Pickler, from the beginning, is
    ...
    try:
        ppers_func[0] = NULL
        return ___Pickler_xsave(pctx, mod, self, obj, pers_func)
    finally:
...
cdef int __Pickler_xsave_psave0(_pickle_PatchCtx* pctx, PickleState* mod, PicklerObject* self, PyObject* obj) except -1:
...
cdef int ___Pickler_xsave(_pickle_PatchCtx* pctx, PickleState* mod, PicklerObject* self, PyObject* obj, PyObject* pers_func) except -1:
    # persistent reference
    if pers_func != NULL:
        st = __Pickler_xsave_pers(pctx, mod, self, obj, pers_func)
        if st != 0:
            return st
    ...
    return save_invoke(pctx.Pickler_save_orig.addr, pctx.Pickler_save_orig.cconv, mod, self, obj)
...
cdef int __Pickler_xsave_pers(_pickle_PatchCtx* pctx, PickleState* mod, PicklerObject* self, PyObject* obj, PyObject* pers_func) except -1:
    ...
    cdef int bin = (<int*>((<byte*>self) + pctx.iPickler.off_bin))[0]
    if bin:
        __Pickler_xsave(pctx, mod, self, <PyObject*>pid, 1)
...
cdef PicklerTypeInfo _detect_Pickler_typeinfo(pyPickler) except *:
    ...
    for n in range(1,10):
        ...
        class InspectWhilePickling:
            def __init__(self, elemsize, typecast):
                self.elemsize = elemsize
                self.typecast = typecast
                self.diff     = None  # []off that changes
                self.doff2val = {}    # off from .diff  ->  value read from it

            def __reduce__(self):
                self.diff = obj_diff(self.elemsize)
                for off in self.diff:
                    bval = PyBytes_FromStringAndSize(<char*>bobj + <Py_ssize_t>off, self.elemsize)
                    self.doff2val[off] = self.typecast(bval)
                return (int, ())    # arbitrary

        # InspectMaxOutputLen
        # - sees which fields are candidates for max_output_len
        class InspectMaxOutputLen(InspectWhilePickling):
            def __init__(self):
                def mem_as_Py_ssize_t(const byte[::1] bval):
                    assert len(bval) == sizeof(Py_ssize_t)
                    return (<Py_ssize_t*>(&bval[0]))[0]
                super(InspectMaxOutputLen, self).__init__(sizeof(Py_ssize_t), mem_as_Py_ssize_t)
            ...
            def __reduce__(self):
                ...
                return super(InspectMaxOutputLen, self).__reduce__()

        pyobj.__init__(Null(), 0)
        i = InspectMaxOutputLen()
        o += (i,)
        obj_copy()
        pyobj.dump(o)
    ...
    #
    # NOTE py3.13+ sets .persistent_id_attr, but .persistent_id, that save
    #      actually uses, is set by dump() at runtime:
    #       https://github.com/python/cpython/commit/89cee94b315c
    #       https://github.com/python/cpython/commit/223d3dc554dd
    ...
    pyPickler2.persistent_id = staticmethod(persid_func)
    pyobj.__init__(Null())
    i = InspectWhilePickling(sizeof(PyObject*), lambda _: _)
    pyobj.dump(i)
    assert i.diff is not None
    t.off_pers_func = 0
    for off in i.diff:
        if off in busy:
            continue
        if mem_as_PyObjectPtr(i.doff2val[off]) == <PyObject*>persid_func:
            if t.off_pers_func == 0:
                t.off_pers_func = off
            else:
                raise AssertionError("TODO: pers_func is potentially aliased")
    assert t.off_pers_func != 0
    markbusy(t.off_pers_func, sizeof(PyObject*))
    trace('.pers_func:\t', t.off_pers_func)

    # is there .persistent_id_attr ?
    obj_copy()
    pyPickler.__setattr__(pyobj, 'persistent_id', persid_func)
    dpersid_attr = obj_diff(sizeof(PyObject*))
    assert len(dpersid_attr) <= 1
    off_pers_func_attr = -1
    if len(dpersid_attr) == 1:
        off_pers_func_attr = dpersid_attr[0]
        assert (<PyObject**>(bobj + <Py_ssize_t>off_pers_func_attr))[0] == <PyObject*>persid_func
    trace('pers_func_attr:\t', off_pers_func_attr)
    ...
    #   * if it is only 1 pointer - there is no .pers_func_self  (e.g. zodbpickle, py3.13+)
    #   * if it is 2 pointers     - .pers_func_self is there and it is reset to NULL
    class pyPickler3(pyPickler):
        def persistent_id(self, obj): pass
    assert isinstance(pyPickler3, type)
    cdef PyTypeObject*   Pickler3  = <PyTypeObject*>   pyPickler3
    cdef _XPyTypeObject* xPickler3 = <_XPyTypeObject*> pyPickler3
    assert Pickler3.tp_basicsize >= t.size
    assert Pickler3.tp_itemsize  == 0

    pyobj = pyPickler3(Null())
    obj = <PyObject*>pyobj
    assert obj.ob_type == Pickler3
    bobj = <byte*>obj

    obj_copy()
    del pyPickler3.persistent_id
    ...
    if len(dpersid_meth) == 1:
        ...
        assert (<PyObject**>(bobj + <Py_ssize_t>dpersid_meth[0]))[0] == <PyObject*>persid_func
        if off_pers_func_attr == -1:
            assert dpersid_meth[0] == t.off_pers_func,  dpersid_meth
    else:
...
cdef PyObject* mem_as_PyObjectPtr(const byte[::1] bval):
    assert len(bval) == sizeof(PyObject*)
    return (<PyObject**>(&bval[0]))[0]
...
cdef SaveFunc _find_Pickler_save(pymod, pyPickler) except *:
    cdef SaveFunc save
    save.addr  = __find_Pickler_save(pyPickler)
    save.cconv = __detect_save_callconv(pymod, pyPickler, save.addr)
...
cdef extern from * nogil: # see _golang_str_pickle_gpy.S for details
    """
    extern "C" {
...
# in a register or on the stack for unrelated reason. Due to this
# __Pickler_xsave does not rely on pers_save value at all in its control flow.
#
# Starting from py3.12 save gained PickleState* argument. That is also automatically detected.
cdef Callconv __detect_save_callconv(pymod, pyPickler, void* save) except *:
    for p in saveprobe_test_ccv:
        #print("save: probing %s" % callconv_str(p.cconv))
        good = __save_probe1(pymod, pyPickler, save, p.addr)
        #print("  ->", good)
        if good:
            return p.cconv
...
cdef bint __save_probe1(pymod, pyPickler, void* save, void* cfunc) except *:
    ...
    global saveprobe_ncall, saveprobe_mod, saveprobe_modok, saveprobe_self, saveprobe_obj, saveprobe_pers_save
    saveprobe_modok = XPyModule_GetState(pymod)
    saveprobe_ncall = 0
    saveprobe_mod   = <void*>0xbaaaaaad
    saveprobe_self  = NULL
    saveprobe_obj   = NULL
    saveprobe_pers_save = 0xdeadbeaf
    ...
    good = (saveprobe_mod  == saveprobe_modok   and \
            saveprobe_self == <void*>p          and \
...
cdef extern from * nogil:
    ...
    static void*  saveprobe_mod;
    static void*  saveprobe_modok;
    static void*  saveprobe_self;
    static void*  saveprobe_obj;
    static int    saveprobe_pers_save;

    static int saveprobe(void* mod, void* self, PyObject* obj, int pers_save) {
        saveprobe_ncall++;
        saveprobe_mod  = mod;
        saveprobe_self = self;
        saveprobe_obj  = obj;
    ...
    #define DEF_SAVEPROBE_builtin(ccname, callconv)                                 \
        ...
        saveprobe_##ccname(void* self, PyObject* obj, int pers_save) {              \
            return saveprobe(saveprobe_modok, self, obj, pers_save);                \
        }
    #define DEF_SAVEPROBE_builtin_mod(ccname, callconv)                             \
        static int callconv                                                         \
        saveprobe_##ccname(void* mod, void* self, PyObject* obj, int pers_save) {   \
            return saveprobe(mod, self, obj, pers_save);                            \
        }
    #define DEF_SAVEPROBE_builtin_psave0(ccname, callconv)                          \
        static int callconv                                                         \
        saveprobe_##ccname(void* self, PyObject* obj) {                             \
            return saveprobe(saveprobe_modok, self, obj, 0);                        \
        }
    #define DEF_SAVEPROBE_builtin_mod_psave0(ccname, callconv)                      \
        static int callconv                                                         \
        saveprobe_##ccname(void* mod, void* self, PyObject* obj) {                  \
            return saveprobe(mod, self, obj, 0);                                    \
        }
...
cdef extern from *:
    ...
    #define CC_SAVE_DEFCALL1_builtin_mod(ccname, callconv)
    #define CC_SAVE_DEFCALL1_builtin_psave0(ccname, callconv)
    #define CC_SAVE_DEFCALL1_builtin_mod_psave0(ccname, callconv)
    ...
    static int save_invoke(void* save, Callconv cconv, PickleState* mod, void* self, PyObject* obj) {
        using namespace golang;
    ...
    #define CC_SAVE_CALL1_builtin(ccname, callconv)  \
        case CALLCONV_ ## ccname:                                                   \
            return ((int (callconv *)(void*, PyObject*, int))save)                  \
                    (self, obj, pers_save);
    #define CC_SAVE_CALL1_builtin_mod(ccname, callconv)  \
        case CALLCONV_ ## ccname:                                                   \
            return ((int (callconv *)(PickleState*, void*, PyObject*, int))save)    \
                    (mod, self, obj, pers_save);
    ...
    #define CC_SAVE_CALL1_builtin_psave0(ccname, callconv)  \
        case CALLCONV_ ## ccname:                                                   \
            return ((int (callconv *)(void*, PyObject*))save)                       \
                    (self, obj);
    #define CC_SAVE_CALL1_builtin_mod_psave0(ccname, callconv)  \
        case CALLCONV_ ## ccname:                                                   \
            return ((int (callconv *)(PickleState*, void*, PyObject*))save)         \
                    (mod, self, obj);
    ...
    """
    int save_invoke(void* save, Callconv cconv, PickleState* mod, void* self, PyObject* obj) except -1
...
include '_golang_str_pickle_gpy_test.pyx'
...
cdef extern from "Python.h":
    r"""
    static void* XPyModule_GetState(PyObject* mod) {
    #if PY_MAJOR_VERSION >= 3
    ...
    """
    void* XPyModule_GetState(object mod) except? NULL
