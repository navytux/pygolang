...
from golang import go, chan, select, default, nilchan, _PanicError, func, \
                panic, defer, recover, gimport
...
def tgopath():
    ...
    os.environ['GOPATH'] = '%s/testdata' % (dirname(__file__),)
    yield
    if gopath is None:
...
def test_gimport_module(tgopath):
...
def test_gimport_package(tgopath):
